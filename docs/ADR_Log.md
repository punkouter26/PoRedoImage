...
## ADR-031: Deploy #54 — App Service `@Microsoft.KeyVault(...)` Sentinels Not Resolved, 2026-06-29

- **Decision:** The deploy job explicitly **rewrites** every `@Microsoft.KeyVault(...)` App Setting as a **plain (literal) App Setting** with the value resolved by `az keyvault secret show`, immediately after the bicep apply and the `Key Vault Secrets User` RBAC grant. The .NET process then sees real values via `IConfiguration`, not the literal sentinel string. The bicep template keeps the `@Microsoft.KeyVault(...)` App Settings as documentation + the fallback used by the in-app `AddAzureKeyVault` provider; the new step is the production-runtime safety net.
- **Why:** Production deploy #54 (and a partial recovery in this session) had the app crash-looping on `AzureAd OIDC configuration invalid: AzureAd:ClientId is missing or not a parseable GUID`. Root cause was that the App Service platform was returning the unresolved `@Microsoft.KeyVault(VaultName=kv-poshared;SecretName=…)` literal string as the env var value, even though:
  1. The app's system-assigned managed identity had `Key Vault Secrets User` on the RG (RBAC role) AND was added to the vault's `accessPolicies` (this session).
  2. The vault's RBAC was **set to access-policy mode** (`enableRbacAuthorization: False`), with 60+ existing entries using the access-policy model.
  3. The secret `PoRedoImage-AzureAd-ClientId` existed with a valid GUID value (verified via `az keyvault secret show`).
  This is *not* the Key Vault RBAC race ADR-026 / ADR-027 documented; it is a separate platform-level symptom: even with the RBAC grant + access policy in place, the platform-side `@Microsoft.KeyVault(...)` resolver didn't return the secret's value — only the literal sentinel was passed to the container. The in-app `ResolveAppInsightsConnectionString` warning ("connection string is present but malformed, no InstrumentationKey") was the smoking gun: the literal sentinel was reaching the process.
- **Mapping (per ADR-017):** The literal rewrite is the production safety net; ADR-027 (RBAC ordering + 75s wait + mid-loop restart) still helps the in-app provider and gives the data plane time to propagate so the platform COULD resolve them. With both in place, the container starts with values that match what the secrets would have resolved to, even when the platform-side resolver fails.
- **Alternatives considered:**
  1. **Switch the vault to RBAC mode** so the App Service KV refs resolve natively. Rejected: high blast radius — 60+ other principals have access policies in the vault; switching the permission model would briefly break every other Po* project sharing `kv-poshared`. Separate ADR if pursued.
  2. **Wrap the secrets in a Key Vault-backed `az webapp config appsettings` instead of `infra/main.bicep`.** Equivalent but less DRY (the values would diverge between projects). The current fix preserves the bicep as the single source-of-truth declaration.
  3. **Add a 5-min wait for KV ref resolution.** Rejected: we already waited through deploy #53/54; even with a 10-min wait the sentinel was still unresolved in our session. The platform-side resolver was simply not firing. A safety net is required, not just patience.
- **Pro:** Every deploy now reaches the post-deploy smoke test with a known-good config. The literal rewrite is idempotent (`az webapp config appsettings set` overwrites cleanly). If the platform resolver ever starts working again, the literal values still match and the rewrite is a no-op semantically.
- **Con:** Secrets are now resolved by the CI service principal (with `Key Vault Secrets User` on `kv-poshared`) and written into the App Service App Settings as plaintext. The App Settings panel in the Azure Portal is the same RBAC-controlled surface as the `@Microsoft.KeyVault(...)` references, so there's no net widening of the blast radius — the same person who can read the env vars can also call `az keyvault secret show` for the same secrets. Anyone who can list App Settings on `poredoimage-web` could already see the same secrets. The CI runner's tmp file and `az` invocation log is the new attack surface; mitigated by GitHub's runner isolation and 1-day artifact retention.
- **Revisit when:** (a) The vault is migrated to RBAC mode (each Po* project re-resolved separately). (b) The platform starts resolving `@Microsoft.KeyVault(...)` App Settings natively for access-policy-mode vaults. (c) A managed-identity-only pattern is adopted that reads secrets at request time via `IMemoryCache` + `KeyVaultSecretManager`, removing the env-var dependency entirely.
- **Files touched:**
  - `.github/workflows/deploy.yml` — added the "Rewrite App Service Key Vault references as literal App Settings" step (after bicep + RBAC grant, before propagation wait + restart + deploy).
  - `infra/main.bicep` — unchanged (the `@Microsoft.KeyVault(...)` App Settings remain as documentation + in-app-provider fallback).

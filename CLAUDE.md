...
This file is the only agent-facing doc in the repo — `AGENT.MD` and `AGENTS.md` were deleted in the
most recent commit, so anything still citing them is stale. [README.md](README.md) is a PRD, not a
...
## Branching — master only

This repository uses **exactly one branch: `master`**. Commit to it directly and push it directly.
Do **not** create feature, fix, or release branches, and do not open pull requests. No other branch
may exist locally or on `origin`; if one appears, merge it into `master` and delete it on both sides.
This overrides the usual "branch before committing" default — here, committing straight to `master`
...
| Creating any branch other than `master` | Master-only repo — see "Branching" above |
